(function(window, document, $, undefined) {
    ...
    var defaults = {
        selector: "#captcha",
        inputValueSelector: '#input-value',
        text: null,
        randomText: true,
        randomColours: true,
        width: 200,
    ...
    CAPTCHA.prototype = {
        ...
        validate: function(userText) {
            if (userText === this._settings.text) {
                ...
                this._container.find('.user-text').attr('data-allow', true)
            } else {
                this._settings.onFailure();
                this._container.find('.user-text').attr('data-allow', false)
            }
        },
        ...
        _generateRandomText: function() {
            const valueInputCaptcha = document.querySelector(this._settings.inputValueSelector);
            this._settings.text = '';
            var length = 6;
            for (var i = 0; i < length; i++) {
                this._settings.text += possibleCharacters.charAt(Math.floor(Math.random() * possibleCharacters.length));
            }
            if (valueInputCaptcha) {
                valueInputCaptcha.value = this._settings.text;
            }
        },
